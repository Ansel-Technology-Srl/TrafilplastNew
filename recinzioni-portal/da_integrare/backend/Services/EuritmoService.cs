...
public class EuritmoService
{
    ...
    public async Task<EuritmoResult> GeneraFileOrdine(int ordNum)
    {
        try
        {
            ...
            var righe = await _db.OrdPrevDett
    ...
    private string RecBGM(OrdPrevTst o)
    {
    ...
    private string RecNAB(OrdPrevTst o, Clienti? c)
    {
    ...
    private string RecNAD(OrdPrevTst o, PuntiDiVendita? p)
    {
        var s = new StringBuilder(305);
        s.Append(A("NAD", 3));
    ...
    private string RecNAI(OrdPrevTst o)
    {
        var s = new StringBuilder(305);
        s.Append(A("NAI", 3));
    ...
    private string RecDTM(OrdPrevTst o)
    {
    ...
    private string RecPAT(OrdPrevTst o)
    {
    ...
    private string RecLIN(OrdPrevDett r, int n)
    {
        ...
        s.Append(Dec(r.Quantita, 12, 3));            // QTAORD
        string um = UM(r.PrdUm);
        s.Append(A(um, 3));                          // UDMQORD
        s.Append(Dec(r.PrezzoUnitario, 12, 3));      // PRZUNI
    ...
    // Alfanumerico: tronca o pad dx con spazi
    ...
    // Numerico intero: zeri a sinistra
    private static string N(int v, int len)
        => v.ToString().PadLeft(len, '0')[..len];

    // Numerico decimale: intDigits zeri sx + decDigits
    ...
    // Fallback valore ordine → valore PdV
