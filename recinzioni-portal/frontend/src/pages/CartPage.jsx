  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <PackageOpen className="w-24 h-24 text-gray-300 dark:text-gray-600 mb-6" />
        <h2 className="text-2xl font-semibold text-gray-500 dark:text-gray-400 mb-2">
          {t('cart.emptyCart')}
        </h2>
        <p className="text-gray-400 dark:text-gray-500 mb-6">{t('cart.emptyCartMessage')}</p>
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-3">
          ...
          {t('cart.title')}
          <span className="text-sm font-normal bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-full">
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm" aria-label={t('a11y.tableCaption.cart')}>
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800 border-b dark:border-gray-700 text-left text-xs text-gray-500 dark:text-gray-400 uppercase">
                  ...
                  <Fragment key={item.id}>
                    ...
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-gray-400 dark:text-gray-500">{item.prdCod}</div>
                        {isConfigured && (
                          <span className="inline-flex items-center text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded mt-1">
                            <Settings2 className="w-3 h-3 mr-1" />
                            {t('cart.configuredProduct')}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center text-gray-500 dark:text-gray-400">{item.prdUm}</td>
                      <td className="px-4 py-3">
                        {isConfigured ? (
                          <div className="text-center text-gray-500 dark:text-gray-400" title={t('cart.quantityFixed')}>
                            {item.quantita}
                          </div>
                        ) : (
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <input
                              ...
                              className="w-14 text-center border dark:border-gray-600 rounded px-1 py-0.5 text-sm dark:bg-gray-700"
                            />
                            <button
                    ...
                    {isConfigured && isExpanded && (
                      <>
                        ...
                        {item.componenti?.map((comp, idx) => (
                          <tr key={`${item.id}-comp-${idx}`} className="bg-blue-50/50 dark:bg-blue-900/20 text-sm">
                            <td></td>
                            <td className="px-4 py-2 pl-10 text-gray-600 dark:text-gray-400">
                              <span className="text-gray-400 dark:text-gray-500 mr-1">└</span>
                              {comp.prdDes}
                              <span className="text-xs text-gray-400 dark:text-gray-500 ml-2">{comp.prdCod}</span>
                            </td>
                            <td className="px-4 py-2 text-center text-gray-500 dark:text-gray-400 text-xs">{comp.prdUm}</td>
                            <td className="px-4 py-2 text-center text-gray-500 dark:text-gray-400">{comp.quantita}</td>
                            <td className="px-4 py-2 text-right text-gray-500 dark:text-gray-400">{fmt.currency(comp.prezzoUnitario || 0)}</td>
                            <td className="px-4 py-2 text-right text-gray-600 dark:text-gray-400">{fmt.currency(comp.prezzoTotale || 0)}</td>
                            <td></td>
                          </tr>
                        ))}
                      </>
      ...
      <div className="mt-6 flex flex-col md:flex-row justify-between items-start gap-4">
        ...
        <div className="card p-5 min-w-[280px]">
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-500 dark:text-gray-400">{t('cart.vat')}</span>
