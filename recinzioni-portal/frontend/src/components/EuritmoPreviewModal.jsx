...
export default function EuritmoPreviewModal({ ordNum, isOpen, onClose, onSent }) {
  ...
  const loadPreview = async () => {
    ...
    setError(null);
    try {
      const res = await api.get(`/ordini/${ordNum}/euritmo/preview`);
      if (res?.success) setData(res.data);
      else setError(res?.message || t('euritmo.loadError'));
    } catch (err) {
      setError(t('euritmo.loadError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownload = async () => {
    try {
      const blob = await api.downloadBlob(`/ordini/${ordNum}/euritmo`);
      ...
      toast.success(t('euritmo.downloadSuccess'));
    } catch {
      toast.error(t('euritmo.downloadError'));
    }
  };

  const handleSend = async () => {
    const isResend = data?.sent;
    const confirmMsg = isResend ? t('euritmo.confirmResend') : t('euritmo.confirmSend');
    if (!window.confirm(confirmMsg)) return;

    setSending(true);
    try {
      const res = await api.put(`/ordini/${ordNum}/invia`);
      if (res?.success) {
        toast.success(t('euritmo.sendSuccess'));
        setData(prev => prev ? { ...prev, sent: true, sentAt: res.data?.sentAt } : prev);
        onSent?.();
      } else {
        toast.error(res?.message || t('euritmo.sendError'));
      }
    } catch (err) {
      toast.error(t('euritmo.sendError'));
    } finally {
      setSending(false);
    }
  };
      ...
      <div ref={modalRef} className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col" role="dialog" aria-modal="true" aria-labelledby="euritmo-modal-title">
          ...
          <div className={`mx-6 mt-4 px-4 py-2 rounded-lg flex items-center gap-2 text-sm ${
            ...
            {data.sent ? (
              <>
                ...
                {t('euritmo.sentAt', {
                  date: fmt.date(data.sentAt),
                  time: new Date(data.sentAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                })}
              </>
            ) : (
              <>
            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(RECORD_COLORS).map(([code, c]) => (
                <span key={code} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs ${c.bg} ${c.text}`}>
                  <strong>{code}</strong> {t(`euritmo.recordTypes.${code}`, c.label)}
                </span>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-xl">
          ...
          <button
            ...
            className={`flex items-center gap-2 ${data?.sent ? 'btn-secondary' : 'btn-primary'}`}
          >
            {sending ? <Loader2 className="w-4 h-4 animate-spin" />
              : data?.sent ? <RefreshCw className="w-4 h-4" />
              : <Send className="w-4 h-4" />}
            {data?.sent ? t('euritmo.resend') : t('euritmo.send')}
          </button>
        </div>
      </div>
    </div>
